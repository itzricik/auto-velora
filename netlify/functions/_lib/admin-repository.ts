...
export async function listAdminBookings(
  ...
  if (search) {
    ...
    const terms = [
      ...
      `vehicle_description.ilike.*${search}*`,
  ...
  const rows = await db.request<AdminBooking[]>(
    `/rest/v1/bookings?${filters.join('&')}${filters.length ? '&' : ''}select=id,reference,status,starts_at,ends_at,estimated_price_cents,estimated_duration_minutes,final_price_cents,vehicle_description,customer_notes,internal_notes,booking_language,idempotency_key,customers(full_name,normalized_email,normalized_phone),booking_services(service_name_snapshot,unit_price_cents_snapshot,duration_minutes_snapshot)&order=starts_at.${input.sort}&offset=${offset}&limit=${input.pageSize + 1}`,
  )
...
export async function getAdminBooking(db: SupabaseServer, id: string): Promise<AdminBooking | null> {
  const rows = await db.request<AdminBooking[]>(
    `/rest/v1/bookings?id=eq.${id}&select=id,reference,status,starts_at,ends_at,estimated_price_cents,estimated_duration_minutes,final_price_cents,vehicle_description,customer_notes,internal_notes,booking_language,idempotency_key,customers(full_name,normalized_email,normalized_phone),booking_services(service_name_snapshot,unit_price_cents_snapshot,duration_minutes_snapshot),booking_status_history(previous_status,new_status,change_source,note,created_at)&booking_status_history.order=created_at.asc&limit=1`,
  )
  return rows[0] ?? null
}
...
export async function getAdminSummary(db: SupabaseServer, now = new Date()) {
  ...
  const rows = await db.request<Array<{ status: BookingStatus; starts_at: string }>>(
    `/rest/v1/bookings?starts_at=gte.${encodeURIComponent(from.toISOString())}&starts_at=lt.${encodeURIComponent(until.toISOString())}&select=status,starts_at&limit=5000`,
  )
  ...
  return {
    ...
    upcoming: rows.filter((row) => Date.parse(row.starts_at) >= now.getTime() && !['cancelled', 'rejected'].includes(row.status)).length,
