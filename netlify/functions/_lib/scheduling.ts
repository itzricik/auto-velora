...
export async function getAvailableSlots(
  ...
  const [bays, hours, blocks, bookings] = await Promise.all([
    ...
    db.request<BusyPeriod[]>(`/rest/v1/bookings?status=in.(confirmed,in_progress)&starts_at=lt.${encodeURIComponent(dayEnd)}&ends_at=gt.${encodeURIComponent(dayStart)}&select=work_bay_id,starts_at,ends_at`),
  ])
