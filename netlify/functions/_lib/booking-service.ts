...
const messages = {
  en: {
    requested: 'Your booking request was created and is awaiting studio confirmation.',
    confirmed: 'Your booking was confirmed.',
  },
  lv: {
    requested: 'Jūsu vizītes pieprasījums ir izveidots un gaida studijas apstiprinājumu.',
    confirmed: 'Jūsu vizīte ir apstiprināta.',
  },
  ru: {
    requested: 'Ваш запрос на запись создан и ожидает подтверждения студией.',
...
export async function createProfessionalBooking(input: {
  ...
  const desiredStatus = input.approvalMode === 'automatic' ? 'confirmed' : 'requested'
  let booking
    ...
    try {
      booking = await createBookingTransaction(input.db, {
        ...
        p_service_snapshots: estimate.services.map((service) => ({
          ...
          unit_price_cents: service.basePriceCents,
  ...
  return {
    ...
    managementUrl: `${input.publicSiteUrl}/booking?reference=${encodeURIComponent(booking.reference)}&token=${encodeURIComponent(accessToken)}`,
