  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose() }}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="reservation-title">
    ...
    <form onSubmit={submit}>
      ...
      <div className="form-grid">
        ...
        <label>Source<select value={props.reservation?.source === 'public_website' || props.reservation?.source === 'telegram' ? props.reservation.source : form.source} disabled={props.reservation?.source === 'public_website' || props.reservation?.source === 'telegram'} onChange={(event) => update('source', event.target.value as ReservationInput['source'])}>{props.reservation?.source === 'public_website' && <option value="public_website">Public website</option>}{props.reservation?.source === 'telegram' && <option value="telegram">Telegram</option>}<option value="admin">Admin</option><option value="phone">Phone</option><option value="walk_in">Walk-in</option></select></label>
      </div>
