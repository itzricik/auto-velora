const en = {
  ...
  privacy: {
    ...
    processingBody: 'In the current portfolio demo, information stays only in the temporary form state and is not sent or stored. Google Forms processing is available only if the submission mode is deliberately changed after the required privacy configuration is completed.',
...
const lv: typeof en = {
  ...
  privacy: {
    ...
    processingTitle: 'Apstrādes pakalpojumi', processingBody: 'Pašreizējā portfolio demonstrācijā informācija paliek tikai veidlapas pagaidu stāvoklī un netiek nosūtīta vai saglabāta. Google Forms apstrāde ir pieejama tikai pēc apzinātas iesniegšanas režīma maiņas un obligātās privātuma konfigurācijas pabeigšanas.',
...
const ru: typeof en = {
  ...
  privacy: {
    ...
    processingTitle: 'Сервисы обработки', processingBody: 'В текущей демонстрации для портфолио информация остаётся только во временном состоянии формы и не отправляется и не сохраняется. Обработка через Google Forms доступна только после намеренного изменения режима отправки и заполнения обязательных настроек конфиденциальности.',
