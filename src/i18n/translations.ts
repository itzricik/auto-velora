...
const lv: typeof en = {
  ...
  privacy: {
    ...
    processingTitle: 'Apstrādes pakalpojumi', processingBody: 'Pārlūks mēģina nosūtīt pieteikumu uz Google Forms. Google Forms un saistīta Google izklājlapa var apstrādāt un glabāt studijas datus.',
...
const ru: typeof en = {
  ...
  booking: {
    eyebrow: 'Запрос на запись', title: 'Расскажите об автомобиле.', body: 'Отправьте запрос на рассмотрение. Он остаётся неподтверждённым, пока мы отдельно не согласуем доступность, объём и цену.', pendingNotice: 'Эта форма отправляет запрос, а не подтверждённую запись. Окончательное подтверждение поступит отдельно по телефону или электронной почте.', name: 'Имя и фамилия', phone: 'Номер телефона', email: 'Электронная почта', vehicle: 'Марка и модель автомобиля', services: 'Выбранные услуги', date: 'Желаемая дата', message: 'Дополнительное сообщение', consent: 'Я разрешаю VELORA использовать эти данные, чтобы ответить на мой запрос.', optional: 'Необязательно',
  ...
  privacy: {
    ...
    processingTitle: 'Сервисы обработки', processingBody: 'Браузер пытается отправить запрос в Google Forms. Google Forms и связанная Google Таблица могут обрабатывать и хранить данные для студии.',
