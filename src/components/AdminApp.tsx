          ...
          <section className="admin-panel">
            <div className="admin-filters">
              ...
              <select aria-label="Status filter" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
                ...
                {['new', 'contacted', 'confirmed', 'in_progress', 'completed', 'cancelled', 'no_show'].map((status) => <option key={status}>{status}</option>)}
              </select>
      ...
      {selected && (
        <aside className="admin-detail" aria-label="Booking details">
          ...
          <div className="admin-actions">
            {selected.status === 'new' && <><button onClick={() => applyAction({ bookingId: selected.id, status: 'contacted' })}>Mark contacted</button><button onClick={() => applyAction({ bookingId: selected.id, status: 'confirmed' })}>Confirm</button></>}
            {selected.status === 'contacted' && <button onClick={() => applyAction({ bookingId: selected.id, status: 'confirmed' })}>Confirm</button>}
            ...
            {['new', 'contacted', 'confirmed', 'in_progress'].includes(selected.status) && <button onClick={() => applyAction({ bookingId: selected.id, status: 'cancelled' })}>Cancel</button>}
          </div>
