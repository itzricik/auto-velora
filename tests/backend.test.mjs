...
import createReservationHandler from '../netlify/functions/create-reservation.ts'
...
describe('public request boundary', () => {
  ...
  test('accepts an available start but discards browser prices and durations', () => {
    ...
    assert.equal(parsed.success, true)
    if (!parsed.success) return
    assert.equal(parsed.data.requestedStart, '2030-01-02T07:00:00.000Z')
    assert.equal('estimatedPriceCents' in parsed.data, false)
    assert.equal('durationMinutes' in parsed.data, false)
  })
...
describe('authoritative pricing and transaction payload', () => {
  ...
  test('calculates integer cents and item snapshots', () => {
    ...
    assert.equal(estimate.durationMinutes, 525)
    ...
    const db = {
      async request(path, init = {}) {
        ...
        if (path === '/rest/v1/rpc/scheduling_availability') return {
          ...
          nearest: null,
          slots: [{
            start: validRequest.requestedStart,
            end: '2030-01-02T09:00:00.000Z',
            durationMinutes: 120,
            availableBayCount: 3,
            segments: [{ segment_start: validRequest.requestedStart, segment_end: '2030-01-02T09:00:00.000Z', duration_minutes: 120 }],
          }],
        }
...
describe('duration reservation transaction', () => {
  function durationDatabase({ overnight = false } = {}) {
    let transactionBody
    const db = {
      async request(path, init = {}) {
        if (path.startsWith('/rest/v1/vehicle_categories')) return [{
          id: validRequest.vehicleCategoryId, code: 'compact', name_en: 'Compact', name_lv: 'Kompakts', name_ru: 'Компакт', price_multiplier: 1, duration_multiplier: 1,
        }]
        if (path.startsWith('/rest/v1/services')) return [{
    ...
    return { db, getTransactionBody: () => transactionBody }
  }

  test('rechecks availability and writes server price, duration, snapshots and pending status atomically', async () => {
    const { db, getTransactionBody } = durationDatabase()
    const result = await createPendingReservation({
      request: { ...validReservationRequest, email: 'customer@example.test', phone: '+37120000001', overnightAcknowledged: false },
      db, minNoticeHours: 0, maxDaysAhead: 90, requestId: 'request-id', now: new Date('2029-12-30T00:00:00.000Z'),
    })
    const payload = getTransactionBody()
    assert.equal(result.status, 'pending')
    assert.equal(payload.p_estimated_total_cents, 4500)
    assert.equal(payload.p_calculated_duration_minutes, 120)
    assert.equal(payload.p_service_snapshots[0].service_name, 'Signature Exterior')
    assert.equal(payload.p_package_snapshot, null)
  })

  test('requires overnight acknowledgement before any reservation transaction', async () => {
    const { db, getTransactionBody } = durationDatabase({ overnight: true })
    await assert.rejects(() => createPendingReservation({
      request: { ...validReservationRequest, email: 'customer@example.test', phone: '+37120000001', overnightAcknowledged: false },
      db, minNoticeHours: 0, maxDaysAhead: 90, requestId: 'request-id', now: new Date('2029-12-30T00:00:00.000Z'),
    }), /OVERNIGHT_ACK_REQUIRED/)
    assert.equal(getTransactionBody(), undefined)
  })
})
...
describe('create-reservation endpoint protection', () => {
  const originalFetch = globalThis.fetch
  beforeEach(() => {
    Object.assign(process.env, {
      SUPABASE_URL: 'https://supabase.example.test',
  ...
  function request(body) {
    return new Request('https://autodetailing-velora.netlify.app/api/create-reservation', {
      method: 'POST',
      headers: { Origin: 'https://autodetailing-velora.netlify.app', 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    })
  }
  ...
  test('rejects invalid input before database access', async () => {
    let fetched = false
    globalThis.fetch = async () => { fetched = true; throw new Error('unexpected') }
    const response = await createReservationHandler(request({}))
    assert.equal(response.status, 422)
    assert.equal(fetched, false)
  })

  test('rejects the honeypot before database access', async () => {
    let fetched = false
    globalThis.fetch = async () => { fetched = true; throw new Error('unexpected') }
    const response = await createReservationHandler(request({ ...validReservationRequest, company: 'Spam Ltd' }))
    assert.equal(response.status, 400)
    assert.equal(fetched, false)
  })
})
